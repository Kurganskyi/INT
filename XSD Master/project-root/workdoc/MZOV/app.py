...
<body>
    <header>
        <h1>Извлечь xml из json</h1>
        <div style="color:var(--text-accent); font-size:1.1rem; margin-top:8px;">Утилита для извлечения XML из JSON-файла</div>
    </header>
    <div class="main-flex">
      <div class="left-col">
        {% if notify %}
        ...
        <form class="lego-card" method="post" action="/extract_xml" enctype="multipart/form-data">
            <h2>XSD Master: Импорт JSON → XML</h2>
        ...
        <form class="lego-card" method="post" action="/extract_vars" enctype="multipart/form-data" style="margin-top:0;">
            <h2>XSD Master: Извлечение переменных</h2>
        ...
        <form class="lego-card" method="post" action="/generate_velocity" enctype="multipart/form-data" style="margin-top:0;">
            <h2>XSD Master: Генерация Velocity-шаблона</h2>
        ...
        <form class="lego-card" method="post" action="/generate_velocity_xsd" enctype="multipart/form-data" style="margin-top:0;">
            <h2>XSD Master: Velocity по export_df + XSD</h2>
        ...
        <form class="lego-card" method="post" action="/transliterate_xsd" enctype="multipart/form-data" style="margin-top:0;">
            <h2>XSD Master: Транслитерация XSD</h2>
...
def extract_xml():
    ...
    if request.method == 'POST':
        ...
        if not file or not output_dir:
            ...
            cmd = ['python', 'extract_xml_from_export_task_both.py', temp_json_path, output_dir]
            try:
                ...
                try:
                    ...
                    pass
    return render_template_string(HTML_TEMPLATE, notify=notify, notify_type=notify_type, dirs=dirs, output=output, xml_code=xml_code, vars_output=vars_output, vars_xml_code=vars_xml_code, velocity_output=velocity_output, velocity_code=velocity_code)
...
def extract_vars():
    ...
    if not file or not output_dir:
        ...
        if vars_xml_filename:
            ...
            base_name = os.path.splitext(os.path.basename(temp_json_path))[0] + '.xml'
        last_xml_path = os.path.join(output_dir, base_name)
        cmd = ['python', 'extract_variables_from_export_df.py', temp_json_path, output_dir, base_name]
        try:
            ...
            try:
                ...
                pass
    return render_template_string(HTML_TEMPLATE, notify=None, notify_type=None, dirs=dirs, output=None, xml_code=None, vars_output=vars_output, vars_xml_code=vars_xml_code, velocity_output=velocity_output, velocity_code=velocity_code)
...
def generate_velocity():
    ...
    if not file or not output_dir or not velocity_filename or not rootvar:
        ...
        cmd = ['python', 'generate_velocity_from_export_df.py',
        ...
        try:
            ...
            try:
                ...
                pass
    return render_template_string(HTML_TEMPLATE, notify=None, notify_type=None, dirs=dirs, output=None, xml_code=None, vars_output=None, vars_xml_code=None, velocity_output=velocity_output, velocity_code=velocity_code)
...
def generate_velocity_xsd():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for file_path in xsd_files_saved:
            if file_path.lower().endswith('.zip'):
                try:
                    ...
                    notify_type = 'error'
                    return render_template_string(HTML_TEMPLATE, notify=notify, notify_type=notify_type, dirs=dirs, output=None, xml_code=None, vars_output=None, vars_xml_code=None, velocity_output=None, velocity_code=None, velocity_xsd_output=None)
        ...
        if not main_xsd_name:
            ...
            return render_template_string(HTML_TEMPLATE, notify=notify, notify_type='info', dirs=dirs, output=None, xml_code=None, vars_output=None, vars_xml_code=None, velocity_output=None, velocity_code=None, velocity_xsd_output=None, main_xsd_options=all_xsd_files)
        ...
        last_velocity_path = os.path.join(output_dir, velocity_filename)
        main_xsd_path = os.path.join(xsd_dir_path, str(main_xsd_name))
        cmd = [
            'python', 'generate_velocity_from_export_df_and_xsd.py',
    ...
    return render_template_string(HTML_TEMPLATE, notify=notify, notify_type=notify_type, dirs=dirs, output=None, xml_code=None, vars_output=None, vars_xml_code=None, velocity_output=None, velocity_code=velocity_code, velocity_xsd_output=velocity_xsd_output)
...
def transliterate_xsd():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for f in xsd_dir_files:
                if f and getattr(f, 'filename', None) and f.filename:
                    ...
                    f.save(save_path)
            translit_cmd = [
                'python', 'transliterate_filenames_and_xsd_refs.py',
                '--dir', xsd_dir_path,
    ...
    return render_template_string(HTML_TEMPLATE, notify=None, notify_type=None, dirs=dirs, output=None, xml_code=None, vars_output=None, vars_xml_code=None, velocity_output=None, velocity_code=None, velocity_xsd_output=None, translit_output=translit_output, eng_zip_url=eng_zip_url)
